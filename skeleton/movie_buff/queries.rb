...
def vanity_projects
  ...
  Movie.joins(:castings).joins(:actors).where("movies.director_id = actors.id AND castings.ord = 1").select("movies.id, movies.title, actors.name") #AND castings.ord = 1")
end
...
def starring(whazzername)
  ...
  Actor.find_by("name LIKE ?", "%#{whazzername}%").movies
